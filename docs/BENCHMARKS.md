...
| **Tool-call history arguments (`TOOL-CALL-HISTORY-ARGS`, #526)** | Correctness-only renderer boundary; CPU | `GATING`: fixture RED/GREEN; protocol/serving + mutation review pass, including assistant-only scope. Live Gemma gate pending; no throughput owed | OpenAI wire string preserved |
