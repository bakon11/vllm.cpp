...
| Vulkan vs llama.cpp Vulkan (`BENCH-VK-LLAMA`) | **Both arms measured, same weights.** llama.cpp Vulkan `NV_coopmat2`: pp128 **11,514 t/s**, tg32 **160.9 t/s**. vllm.cpp Vulkan: prefill **137.7**, decode **87.3 t/s** | Decode **8.59 -> 91.7 t/s** (**10.7x**), **1.75x** off llama.cpp; 6/6 exact. Ring 16->128 **1.51x** (8/8); GEMV unroll **1.055x** (7/8) on re-test. paged_attn barrier batching REFUTED (memory-bound) |
